...
final class DiagnosticsProvider {
    func collect() async -> DiagnosticsSnapshot {
        ...
        let toolRows = ToolRegistry.shared.definitions().map { def in
        ...
        let privacy = PrivacyReportSnapshot(localOnlyMode: networkState != PermissionState.granted.rawValue, networkAccessState: networkState, recentToolCategories: Array(Set(toolRows.map(\.category))).sorted(), appIntentLimitations: ["Sensitive actions require open-app approval", "No external network by default"])
