...
actor MicrosoftGraphMailClient {
    ...
    func sendMail(_ mail: GraphSendMailRequest, accessToken: String) async throws {
        ...
        try await retryWithBackoff { [session] in
            let (data, response) = try await session.data(for: request)
    ...
    func uploadLargeAttachment(messageID: String, fileData: Data, fileName: String, contentType: String, accessToken: String) async throws {
        ...
        let uploadSession: GraphUploadSession = try await retryWithBackoff { [session, decoder] in
            let (data, response) = try await session.data(for: sessionRequest)
            try Self.validate(response: response, data: data, allowedStatuses: 200...201)
            return try decoder.decode(GraphUploadSession.self, from: data)
        }
        ...
        while offset < fileData.count {
            ...
            let (_, response) = try await session.data(for: chunkRequest)
            guard let http = response as? HTTPURLResponse else { throw URLError(.badServerResponse) }
            if http.statusCode == 200 || http.statusCode == 201 { break }
            if http.statusCode != 202 { throw GraphHTTPError.unexpectedStatus(http.statusCode) }
            offset = end + 1
        }
    }
