...
struct FinalOutputSanitizerTests {
    ...
    @Test func removesBareSearchResultsJSONPayload() {
        let raw = #"{"kind":"searchResults","mediaKind":"page","results":[]}"#
        let out = FinalOutputSanitizer.sanitizeUserVisibleText(raw)
        #expect(!out.text.localizedCaseInsensitiveContains("\"kind\":\"searchResults\""))
