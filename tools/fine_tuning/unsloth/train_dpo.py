...
def main():
    ...
    if not (d / 'train_dpo.jsonl').exists() or not (d / 'val_dpo.jsonl').exists():
        raise FileNotFoundError(f"Missing DPO dataset split files under {d}: expected train_dpo.jsonl and val_dpo.jsonl")
